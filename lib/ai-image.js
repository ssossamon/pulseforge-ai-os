...
// Netlify Blobs does not auto-inject its environment into these
// (V1-style) functions — confirmed via the actual runtime error and
// Netlify's own troubleshooting docs. Same class of issue as the database
// connection string, same fix: supply siteID + a personal access token
// explicitly. Requires SITE_ID (auto-provided by Netlify) and
// NETLIFY_BLOBS_TOKEN (a personal access token, set manually as an env var).
...
async function generateCampaignImage({ apiKey, prompt }) {
  ...
  try {
    ...
    return { success: false, code: 'STORAGE_NOT_CONFIGURED', error: 'Image storage is not configured on this deployment yet (missing NETLIFY_BLOBS_TOKEN).' };
  }
...
module.exports = { generateCampaignImage, getImageStore };
