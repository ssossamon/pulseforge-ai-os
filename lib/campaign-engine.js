...
function buildPrompt({ inferMode, rawInput, pageText, name, offerUrl, targetAudience, mainBenefit, tone, platforms, modules, language, voice, complianceCheck }) {
  ...
  if (modules.includes('campaign_image')) {
    shape.image_prompt = 'a single vivid, descriptive sentence for an accompanying promotional image';
    notes.push('image_prompt: a scene description for an AI image generator to illustrate this campaign. NEVER include any text, words, letters, or logos to render in the image (image models render text poorly) — describe the visual scene/subject/mood only.');
  }
  if (complianceCheck) {
...
function assetsFromParsed(parsed, complianceCheck) {
  ...
  return { assetsToInsert, complianceNotes, imagePrompt: parsed.image_prompt || null };
}
...
async function autoPostToConnectedAccounts(userId, assetsToInsert) {
  const { postToPlatform } = require('./social-post-helpers');

  const postAssets = assetsToInsert.filter((a) => a.module === 'platform_post');
  const byPlatform = {};
  postAssets.forEach((a) => {
    // Prefer the AI's "stronger pick" version; otherwise keep the first seen.
    if (!byPlatform[a.platform] || a.label.includes('⭐')) byPlatform[a.platform] = a;
  });
  const platforms = Object.keys(byPlatform);
  if (!platforms.length) return [];

  const connResult = await query('SELECT * FROM social_connections WHERE user_id = $1 AND platform = ANY($2)', [userId, platforms]);
  const connByPlatform = {};
  connResult.rows.forEach((c) => { connByPlatform[c.platform] = c; });

  const results = [];
  for (const platform of platforms) {
    const conn = connByPlatform[platform];
    if (!conn) continue; // not connected — skip silently, nothing to report
    const content = byPlatform[platform].content.replace(/\n\n\[AI note:[^\]]*\]$/, '');
    const result = await postToPlatform(platform, conn, content);
    results.push({ platform, ...result });
  }
  return results;
}

module.exports = { buildPrompt, stripFences, assetsFromParsed, persistCampaign, fireZapierWebhook, autoPostToConnectedAccounts, PLATFORM_SPEC, TONE_DESC, LANGUAGE_NAMES };
